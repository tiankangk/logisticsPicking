<template>
    <div class="yjs-home">
        ...
        <popup class="popup-content" v-model="isPopup">
            <popup-header left-text="取消" right-text="确定" title="请选择异常的原因" @on-click-left="handleCancel" @on-click-right="handleOk" ></popup-header>
            <group>
        ...
        watch: {
            pickShopList(newVal) {
                this.shopInforList = [
                    ...
                    {
                        label: "产品批号",
                        value: `<span style="font-weight:bold;color:#0ec713;">${
                            newVal[0].SCPH
                        }</span>`
                    },
                            ...
                            if (res.success) {
                                if (that.pickShopList.length === 1) {
                                    ...
                                    if (Object.keys(that.shopList).length == 0) {
                                        ...
                                        that.areaNum = Object.keys(
                                            that.shopList
                                        )[0];
                                        that.clickRegin(
                                            that.shopList[
                                                Object.keys(that.shopList)[0]
                                            ]
                                        );
                                    }
                                } else {
            ...
            handlePick() {
                let timer = setTimeout(() => {
                    ...
                    let HWBH = this.pickShopList[0].HWBH
                        ? this.pickShopList[0].HWBH.replace(
                              /\s+/g,
                              ""
                          ).toLowerCase()
                        : "";
                        ...
                        pickGoods({
                            ...
                            if (this.pickShopList.length === 1) {
                                ...
                                if (Object.keys(this.shopList).length == 0) {
                                    ...
                                    this.clickRegin(
                                        this.shopList[Object.keys(this.shopList)[0]]
                                    );
                                }
                            } else {
            ...
            sortValFun(prop) {
                return (obj1, obj2) => {
                    if (obj2[prop] && obj1[prop]) {
                        return obj2[prop].slice(4) - obj1[prop].slice(4);
                    }
                };
            },
